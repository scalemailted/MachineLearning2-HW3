...
class NeuralNet:
    ...
    def train(self, X, Y):
        ...
        ga.execute(error_data, epoch_data, betas_data,accuracy)

        self.error_log.append(error_data);
        self.epoch_log.append(epoch_data);
        self.betas_log = betas_data;
    ...
    def testB(self, X, Y):
        ...
        accuracy = []
        self.set_data(X,Y);
        for b in self.betas_log:
            b.get_mse(X,Y,self)
            beta_errs.append(b.score)
            #accuracy.append( self.get_accuracy() );
            accuracy.append( b.accuracy );
        self.test_error_log.append(beta_errs); 
...
class GeneticAlgorithm:
    ...
    #
    def execute(self, error_data, epoch_data, betas_data, accuracy):
        ...
        while self.population[0].score < self.target_value or counter < self.max_generations:
            ...
            self.calculate_fitness();
            counter += 1;
            epoch_data.append(counter)
            error_data.append(self.population[0].score)
            betas_data.append( copy.deepcopy( self.population[0]) );
            #x,y,ann = self.data;
            accuracy.append( self.population[0].accuracy );
...
def main():
    ...
    test_err = np.mean(np.array(ann.test_error_log).T,axis=1)
    epo = ann.epoch_log[0]
    fig = plt.figure()
    fig.suptitle('Neural Net/GA [3 Hidden Layers]');
    plt.plot(epo, train_err, label="Train Error");
    plt.plot(epo, test_err, label='Test Error');
    plt.xlabel('Generations')
    plt.ylabel('Avg Error (10CV)')
    plt.legend()
    plt.savefig('./output/ANN03GA-Error.png')
    #accuracy
    ...
    plt.legend()
    plt.savefig('./output/ANN03GA-Accuracy.png')
