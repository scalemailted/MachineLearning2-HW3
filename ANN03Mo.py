...
class NeuralNet:
    ...
    def train(self, X, Y):
        ...
        while self.mse > self.target_mse and self.epoch < self.max_epoch:
            ...
            self.mse = CSqErr;
            self.epoch += 1;
            self.error_log.append(self.mse);
    ...
    def test(self, X, Y):
        ...
        for j in range(self.Nx):
            self.Z[0] = (np.append(self.X[j],1)).T
            for i in range( len(self.layers)-1 ):
                self.T[i+1] = self.B[i].T @ self.Z[i];
                if (i+1)<len(self.layers)-1:
                    t = 1/(1+np.exp(-self.T[i+1]))
                    self.Z[i+1] = np.append(t,1) #(np.column_stack([t.T,ones])).T
                else:
                    self.Z[i+1] = (1/(1+np.exp(-self.T[i+1])))
        CSqErr += np.sum( np.sum( (self.Y.T - self.Z[-1])**2, 0) ) 
        CSqErr = CSqErr / self.layers[-1]
        CSqErr = CSqErr / self.Nx;
...
def main():
    ...
    epo = ann.epoch_log[0]
    fig = plt.figure()
    fig.suptitle('Neural Net/BP + Momenetum (gamma: '+str(ann.gamma)+') [3 Hidden Layers]');
    plt.plot(epo, train_err, label="Train Error");
    plt.plot(epo, test_err, label='Test Error');
    plt.xlabel('Epochs')
    plt.ylabel('Avg Error (10CV)')
    plt.legend()
    plt.savefig('./output/ANN03Mo-gamma'+str(ann.gamma)+'-Error.png')
    #accuracy
    ...
    plt.legend()
    plt.savefig('./output/ANN03Mo-gamma'+str(ann.gamma)+'-Accuracy.png')
